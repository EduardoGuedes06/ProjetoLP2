...
def atualizar(tabela, conn):
    try:
        ...
        if tabela == 'avaliacao': # Atualizar a tabela avaliação
            ...
            c.execute("UPDATE avaliacao SET materia =?, descricao =?, notaMin =?, notaMax =?, notaAprov =?, dataPrevista =?, idTipo=? WHERE idAvaliacao=?;", avaliacao)
            conn.commit()
        ...
        elif tabela == 'tipo': # Atualizar a tabela tipo
            pesquisar(tabela, conn)
            id = input("Informe o Id do tipo de prova: ")
            descricao = input("Informe a forma de realização: ")

            tipoNovo = (descricao,id)
            c.execute("UPDATE tipo SET descricao=? WHERE idTipo=?;", tipoNovo)
            conn.commit()
